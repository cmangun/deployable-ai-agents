...
describe('Calculator Tool', () => {
  it('adds numbers', async () => {
    const result = await calculatorTool.execute({ operation: 'add', a: 2, b: 3 }) as { result: number };
    expect(result.result).toBe(5);
  });

  it('subtracts numbers', async () => {
    const result = await calculatorTool.execute({ operation: 'subtract', a: 10, b: 4 }) as { result: number };
    expect(result.result).toBe(6);
  });

  it('multiplies numbers', async () => {
    const result = await calculatorTool.execute({ operation: 'multiply', a: 3, b: 4 }) as { result: number };
    expect(result.result).toBe(12);
  });

  it('divides numbers', async () => {
    const result = await calculatorTool.execute({ operation: 'divide', a: 10, b: 2 }) as { result: number };
    expect(result.result).toBe(5);
  });
